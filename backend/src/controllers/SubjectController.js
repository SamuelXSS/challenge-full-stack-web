...
module.exports = {
    ...
    async store(req, res){
        ...
        if(subject){
            return res.status(409).json({ error: `A matéria ${name} já existe!` })
        }
        ...
        return res.status(201).json({ success: 'Matéria criada com sucesso!', createSubject})
    ...
    async update(req, res){
        ...
        if(!subject){
            return res.status(404).json({ error: 'Essa matéria não existe!' })
        } 
    ...
    async destroy(req, res){
        ...
        if(checkGrades){
            return res.status(400).json({ error: 'Você não pode excluir essa matéria pois há notas de alunos atribuídas à ela!' })
        }
